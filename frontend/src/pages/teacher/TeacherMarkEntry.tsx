...
const TeacherMarkEntry: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['teacher-mark-entry', mockTestId],
    queryFn: async () => {
      if (!mockTestId) return Promise.reject('No mock test ID');
      return await teacherApi.getMarkEntryData(mockTestId);
    },
  ...
  const saveMutation = useMutation({
    mutationFn: (marks: any[]) => {
      if (!mockTestId) return Promise.reject('No mock test ID');
      return teacherApi.saveMarks(mockTestId, { marks });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['teacher-mark-entry', mockTestId] });
  ...
  const updateStudentMarks = (studentId: string, field: string, value: any) => {
    setMarksData((prev) =>
      prev.map((student) => {
        ...
        // Recalculate only if sections exist
        let totalMarks = 0;
        let totalPossible = 0;
  ...
  const handleSaveAll = () => {
    const marksToSave = marksData.map((student) => {
      ...
      // Build the mark object with all possible sections, using defaults
      const markObj: any = {
        studentId: student.studentId,
      };

      // Always include all sections with defaults
  ...
  if (isError) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        ...
        <button onClick={() => navigate('/teacher/mock-tests')} className="mt-4 btn-primary">
          Go Back
        </button>
      </div>
    );
