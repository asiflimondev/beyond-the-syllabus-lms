...
const OfficeMarkEntry: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['office-mark-entry', mockTestId],
    queryFn: async () => {
      if (!mockTestId) return Promise.reject('No mock test ID');
      return await officeMockTestApi.getMarkEntryData(mockTestId);
    },
  ...
  const saveMutation = useMutation({
    mutationFn: (marks: any[]) => {
      if (!mockTestId) return Promise.reject('No mock test ID');
      return officeMockTestApi.saveMarks(mockTestId, { marks });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['office-mark-entry', mockTestId] });
  ...
  if (isError) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        ...
        <button onClick={() => navigate('/office/mock-tests')} className="mt-4 btn-primary">
          Go Back
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <button
            onClick={() => navigate('/office/mock-tests')}
...
export default OfficeMarkEntry;